...
use AuthEasy\common\PluginCallbackTask;
use AuthEasy\common\mc;
use AuthEasy\common\MPMU;
use AuthEasy\EventListener;
use AuthEasy\PermsHacker;
use AuthEasy\DbMonitorTask;
class Main extends PluginBase implements Listener,CommandExecutor {
	...
	public function onEnable(){
		...
		$defaults = [
			...
			"# max-attemps" => "kick player after this many login attempts. ",// NOTE: This conflicts with AuthEasy's blockAfterFail setting
			"max-attempts" => 5,
	...
	public function onPlayerCmd(PlayerCommandPreprocessEvent $ev) {
		...
		if (!$this->auth->isPlayerRegistered($pl) || isset($this->chpwd[$n])) {
			if (!isset($this->pwds[$n])) {
				...
				$pl->sendMessage(TextFormat::AQUA.mc::_("Please re-enter pwd"));
				$ev->setCancelled();
				$ev->setMessage("~");
				return;
			}
			if ($this->pwds[$n] != $ev->getMessage()) {
				...
				$ev->setCancelled();
				$ev->setMessage("~");
				$pl->sendMessage(TextFormat::RED.mc::_("The passwords dont match"));
				return;
			}
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args) {
		if (!$this->auth) {
			$sender->sendMessage(TextFormat::RED.mc::_("AuthEasy has been disabled"));
			$sender->sendMessage(TextFormat::RED.mc::_("AuthEasy not found!"));
			return true;
		}
		switch($cmd->getName()){
			case "chpwd":
