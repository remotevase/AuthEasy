...
use AuthEasy\event\PlayerAuthenticateEvent;
use AuthEasy\event\PlayerDeauthenticateEvent;
use AuthEasy\event\PlayerRegisterEvent;
use AuthEasy\event\PlayerUnregisterEvent;
use AuthEasy\provider\DataProvider;
use AuthEasy\provider\DummyDataProvider;
use AuthEasy\provider\MySQLDataProvider;
use AuthEasy\provider\SQLite3DataProvider;
use AuthEasy\provider\YAMLDataProvider;
use AuthEasy\task\ShowMessageTask;

class AuthEasy extends PluginBase{
	...
	protected function removePermissions(PermissionAttachment $attachment){
		...
		unset($permissions["AuthEasy.chat"]);
		unset($permissions["AuthEasy.move"]);
		unset($permissions["AuthEasy.lastid"]);

		//Do this because of permission manager plugins
		if($this->getConfig()->get("disableRegister") === true){
			$permissions["AuthEasy.command.register"] = false;
		}else{
			$permissions["AuthEasy.command.register"] = true;
		}

		if($this->getConfig()->get("disableLogin") === true){
			$permissions["AuthEasy.command.register"] = false;
		}else{
			$permissions["AuthEasy.command.login"] = true;
		}

		uksort($permissions, [AuthEasy::class, "orderPermissionsCallback"]); //Set them in the correct order
