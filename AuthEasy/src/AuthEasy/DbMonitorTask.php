...
use AuthEasy\Main as HelperPlugin;
...
class DbMonitorTask extends PluginTask implements Listener{
  ...
  private function setStatus($mode) {
    ...
    $auth = $this->getOwner()->getServer()->getPluginManager()->getPlugin("AuthEasy");
    if ($auth !== null) {
      $cnt = 0;
  ...
  private function enableAuth($mgr,$auth) {
    ...
    $this->getOwner()->getLogger()->info(mc::_("Enabling AuthEasy"));
  ...
  private function pollDB() {
    ...
    /*
     * let's try to reconnect by resetting AuthEasy
     */
    if ($auth->isEnabled()) {
      $this->getOwner()->getLogger()->info(mc::_("Disabling AuthEasy"));
